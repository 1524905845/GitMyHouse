...
class MultipleChoiceExample(object): # examples for all kind of dataset s
    # including 
    """A single train0ing/test example for the SWAG dataset."""
    def __init__(self,
                 id,
                 context,
                 question,
...
#     '''
#     put commonsense into context,将常识作为上下文
#     '''
...
def load_csqa_dataset(tokenizer, args, data_type, is_training=True):
    ...
    if is_training:
        if args.tpu:
            all_input_ids, all_attention_masks, all_token_type_ids, all_labels = feature_padding(args, data_type,
                                                                                                 all_input_ids,
                                                                                                 all_attention_masks,
                                                                                                 all_token_type_ids,
                                                                                                 all_labels)
        ...
        dataset = TensorDataset(all_input_ids, all_attention_masks, all_token_type_ids,
                                all_labels)  # Dataset wrapping tensors.
    else:
        dataset = TensorDataset(all_input_ids, all_attention_masks, all_token_type_ids)
    # data = {}
    # data["examples"],data["features"],data["dataset"] = examples,features,dataset
