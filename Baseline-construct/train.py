...
def clean_data(cs_str):
    if "The statement " in cs_str:
        ...
        if " helps answer the question " in cs_str:
            ...
            cs_str = cs_str[1]+" because "+cs_str[0]     # 表述的顺序调换
...
def train(args):
    ...
    if args.seed:
        set_seed(args)
    # loading data
    # omcs_corpus = load_omcs(args) # 实验没有利用外部数据
...
# 删除增加外部资源的部分（OMCS）
def make_predictions(args, examples, predictions, attention_scores, data_type="dev"):
...
def eval(args,set_name):
    ...
    tokenizer = select_tokenizer(args)
    examples, _, dataset = load_csqa_dataset(tokenizer, args,set_name, is_training)
    ...
    prediction_json = make_predictions(args,examples,predictions,attention_scores,set_name)
